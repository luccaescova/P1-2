 ...
 */
router.post("/addNewEvent", async (req: Request, res: Response): Promise<any> => {
    try {
 ...
 */
router.get("/getEvents", async (req: Request, res: Response): Promise<any> => {
    try {
        const { status } = req.query; // pending, approved, rejected
        const events = await Event.find({ where: { status: "string" } });
 ...
 */
router.patch("/evaluateEvent", async (req: Request, res: Response): Promise<any> => {
    try {
